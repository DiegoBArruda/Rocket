...
class flight():
    ...
    def simulation2(self, t, y):
        ...
        # Update the velocity
        return self.rocket.acc, mdot,self.rocket.drag, self.rocket.force
...
def rocket_assembly(Mach, Poptim):
    ...
    mass_comb = mass_comb_geral # Mass of the rocket fuel (kg)
...
def main(mass_comb_geral, optimTeste, step=0.01):
    rocket, target = rocket_assembly(2, 101325)
    flight_simulation = flight(rocket, target)
    position = 0
    velocity = 0
    mass = mass_comb_geral
    # Simulate with solve_ivp
    sol = solve_ivp(flight_simulation.simulation, [0, 30], [position, velocity, mass], max_step=step)
    time = sol.t
    altitude = sol.y[0]
    velocity = sol.y[1]
    mass = sol.y[2]
    ...
    dSss = []
    forces = []
    flight_simulation.parachute = False
    for i in range(len(time)):
        ai, mi, di, fi = flight_simulation.simulation2(time[i], [altitude[i], velocity[i], mass[i]])
        acc.append(ai)
        mdots.append(mi)
        dSss.append(di)
        forces.append(fi)
    ...
    # Configurar o número de linhas e colunas para os subgráficos
    fig, axs = plt.subplots(3, 2, figsize=(20, 24))

    # Plot 1 - Altitude vs Time
    axs[0, 0].plot(time, altitude)
    ...
    axs[0, 1].plot(time, velocity)
    axs[0, 1].set_xlabel("Time (s)")
    axs[0, 1].set_ylabel("Velocity (m/s)")
    axs[0, 1].set_title("Velocity vs Time")
    axs[0, 1].grid()

    # Plot 5 - Mass Flow Rate vs Time
    axs[1, 0].plot(time, mdots)
    axs[1, 0].set_xlabel("Time (s)")
    axs[1, 0].set_ylabel("Vazão mássica (kg/s)")
    axs[1, 0].set_title("Vazão mássica vs Time")
    axs[1, 0].grid()
    # Plot 3 - Mass vs Time
    #axs[1, 0].plot(time, mass)
    #axs[1, 0].set_xlabel("Time (s)")
    #axs[1, 0].set_ylabel("Mass (kg)")
    #axs[1, 0].set_title("Mass vs Time")
    #axs[1, 0].grid()

    # Plot 4 - Acceleration vs Time
    axs[1, 1].plot(time, acc)
    axs[1, 1].set_xlabel("Time (s)")
    axs[1, 1].set_ylabel("Aceleração (m/s²)")
    axs[1, 1].set_title("Aceleração vs Time")
    axs[1, 1].grid()

    

    # Plot 6 - Drag vs Time
    axs[2, 0].plot(time, dSss)
    axs[2, 0].set_xlabel("Time (s)")
    axs[2, 0].set_ylabel("Arrasto (N)")
    axs[2, 0].set_title("Arrasto vs Time")
    axs[2, 0].grid()

    # Plot 7 - Force vs Time
    axs[2, 1].plot(time, forces)  # Parece que o gráfico da força está plotando a massa, você deve verificar se é o correto
    axs[2, 1].set_xlabel("Time (s)")
    axs[2, 1].set_ylabel("Força (N)")
    axs[2, 1].set_title("Força Resultante vs Time")
    axs[2, 1].grid()

    # Ajustar os espaços entre os subgráficos
    #plt.tight_layout()
    # Ajustar o espaçamento entre os subgráficos e aumentar o espaço abaixo
...
if __name__ == "__main__":
    massasss = np.linspace(1,10,91)
    hsss = []
    for mc in massasss:
        try:
            soll = main(mc, True, 0.1)
        except:
            soll = main(mc, True, 0.001)
        hsss.append(soll)
        print(f"{mc} - {soll}")
    
    solver, iter = bissecao(main, 3, 4, True)

    fig = plt.figure()
    plt.plot(massasss, hsss)
    plt.scatter(solver, main(solver, True, 0.001), label="Alvo")
    plt.title("Apogeu alcançável vs massa de combustível")
    plt.xlabel("Massa de Combustível (kg)")
    plt.ylabel("Apogeu (m)")
    plt.legend()
    plt.grid()
    plt.savefig("Diego")
    
    main(solver, False, 0.001)